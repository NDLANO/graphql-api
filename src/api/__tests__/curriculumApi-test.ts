...
test('404 when fetching comptence goal should be handled', async () => {
  nock('https://api.test.ndla.no')
    ...
    .reply(
      404,
      '<string xmlns="http://schemas.microsoft.com/2003/10/Serialization/">KM123 ble ikke funnet</string>',
    );
...
test('404 when fetching comptence goal set should be handled', async () => {
  nock('https://api.test.ndla.no')
    ...
    .reply(
      404,
      '<string xmlns="http://schemas.microsoft.com/2003/10/Serialization/">KV123 ble ikke funnet</string>',
    );
